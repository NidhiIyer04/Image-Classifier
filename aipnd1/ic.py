...
#datasets
train_dataset = torchvision.datasets.ImageFolder(root = str(FLOWERS_DIR / Path('train')),
                                        transform=train_transforms)
val_dataset = torchvision.datasets.ImageFolder(root=str(FLOWERS_DIR / Path('valid')),
                                              transform=train_transforms)
...
# 2. Consistency in Function Naming
...
def download_image(url, save_path):
    print_info(f"Downloading image from {url}")
    response = requests.get(url)
    if response.status_code == 200:
        with open(save_path, "wb") as img_file:
            img_file.write(response.content)
        print_info(f"Image saved successfully to {save_path}")
    else:
        print_info(f"Failed to download image. Status code: {response.status_code}")
'''
# model_utils.py
'''
import torch
...
def create_model(arch='resnet34', hidden_layer1=512, num_classes=102):
    if arch == 'resnet34':
        ...
        model.fc = nn.Sequential(
            nn.Linear(512, hidden_layer1),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(hidden_layer1, num_classes)
        )
    else:
        raise ValueError("Unsupported architecture. Choose 'resnet34'.")

    return model

def create_optimizer(model, learning_rate=2e-4):
...
def load_data(data_dir, batch_size):
    ...
    image_datasets = {x: ImageFolder(root=data_dir + '/' + x, transform=data_transforms[x]) for x in ['train', 'val']}

    # Using the image datasets and the trainforms, define the dataloaders
    dataloaders = {x: DataLoader(image_datasets[x], batch_size=batch_size, shuffle=True) for x in ['train', 'val']}

    # Size of training dataset for calculating validation split
...
#from model_utils import create_model, train_model, save_checkpoint
#from utils import load_data, manual_seed
'''
'''
#def main():
    parser = argparse.ArgumentParser(description="Train a new network on a dataset with options to customize training.")
    parser.add_argument('data_dir', type=str, help='Path to the dataset directory')
    parser.add_argument('--save_dir', type=str, default='./', help='Directory to save the checkpoint')
    parser.add_argument('--arch', type=str, default='resnet34', help='Architecture of the pretrained model')
    parser.add_argument('--learning_rate', type=float, default=2e-4, help='Learning rate for training')
    parser.add_argument('--hidden_units', type=int, default=512, help='Number of nodes in the hidden layer')
    ...
    manual_seed()  # Set manual seed for reproducibility
    device = 'cuda' if args.gpu and torch.cuda.is_available() else 'cpu'

    # Load and preprocess data
    train_loader, val_loader, _, _ = load_data(args.data_dir, args.batch_size)

    # Create and train the model
    model = create_model(arch=args.arch, hidden_layer1=args.hidden_units)
    optimizer, criterion = create_optimizer(model, learning_rate=args.learning_rate)
    trained_model, training_stats = train_model(model, train_loader, val_loader, optimizer, criterion, args.epochs, device)

    # Save the trained model as a checkpoint
    save_path = save_checkpoint(trained_model, args.save_dir, args.arch, args.hidden_units)
...
# from utils import process_image, load_class_names
# 
# def main():
...
# 
#     # Make prediction
#     probs, classes = predict(input_image, model, device, topk=args.top_k)
# 
...
#         flower_name = class_to_name[classes[i]]
#         probability = probs[i]
#         print(f"Top {i+1} Prediction: {flower_name} | Probability: {probability*100:.2f}%")
# 
# if __name__ == "__main__":
#     main()
#
